...
{
  if (forward) {
	add_difference(p_modified, difference(p_fixed, orig), scalarTransitionFunc(static_cast<double>(time), static_cast<double>(range)));
  } else {
	add_difference(p_modified, difference(p_fixed, orig), 1.0-scalarTransitionFunc(static_cast<double>(time), static_cast<double>(range)));
  }
}
